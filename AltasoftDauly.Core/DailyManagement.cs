...
namespace AltasoftDaily.Core
{
    ...
    {
        public static List<DailyModel> GetDailyByUserId(int userId)
        {
            ...
            {//01024071605
                var loan = l.GetLoan(AltasoftAPI.LoansAPI.LoanControlFlags.Debts, true, loanId, true);
        ...
        public static long? Add(int docNum, string ccy, DateTime date, string accountIBAN, decimal amount, string purpose, string cashDeskSymbol, int userId)
        {
            #region OrdersService
            AltasoftAPI.OrdersAPI.OrdersService o = new AltasoftAPI.OrdersAPI.OrdersService();
            o.RequestHeadersValue = new AltasoftAPI.OrdersAPI.RequestHeaders() { ApplicationKey = "BusinessCreditClient", RequestId = Guid.NewGuid().ToString() };
